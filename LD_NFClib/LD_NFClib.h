...
class LD_NFC
{
	public:
		...
		void wakeUp();
		uint8_t scanReadTargetID(unsigned char cardbaudrate,unsigned char *targetID, uint8_t * uidLength);
	...
	private: 
		unsigned char readAck(uint16_t timeout);
		char checkDCS(int x);  //  NFC  S50卡  DCS校验检测子函数
		void readReceive(int x);
		unsigned char sendCommandWaitAck(unsigned char *command,unsigned char commandLEN,uint16_t timeout);
